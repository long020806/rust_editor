...
impl CommandBar {
    ...
    pub fn caret_position_col(&self) -> usize {
        ...
        min(max_width, self.size.width as usize)
    }
...
impl UIComponent for CommandBar {
    ...
    fn draw(&mut self, origin: usize) -> Result<(), Error> {
        let area_for_value = self.size.width.saturating_sub(self.prompt.len() as u16); //this is how much space there is between the right side of the prompt and the edge of the bar
        ...
        let value_start = value_end.saturating_sub(area_for_value as usize); //This should give us the start for the grapheme subrange we want to print out.
        let message = format!(
            "{}{}",
            self.prompt,
            self.value.get_visible_graphemes(value_start..value_end)
        );
        let to_print = if message.len() <= self.size.width as usize {
            message
        } else {
            String::new()
        };
