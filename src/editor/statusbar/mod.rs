use super::{terminal::{Terminal, Size}, documentstatus::DocumentStatus, uicomponent::UIComponent};
#[derive(Default)]
...
impl UIComponent for StatusBar {
    ...
    fn draw(&mut self, origin_y: usize) -> Result<(), std::io::Error> {
        ...
        let remainder_len = self.size.width.saturating_sub(beginning.len() as u16) as usize;
        ...
        let to_print = if status.len() <= self.size.width as usize{
            status
        } else {
            String::new()
        };
